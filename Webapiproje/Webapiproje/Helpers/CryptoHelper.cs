...
namespace Webapiproje.Helpers
{
    public class CryptoHelper
    {
        ...
        {
            return Encoding.UTF8.GetBytes(key.PadRight(32).Substring(0, 32)); //.PadRight(32) → Key’i 32 karaktere tamamlar,.Substring(0, 32) → 32 karakteri geçerse keser,Encoding.UTF8.GetBytes(...) → String’i byte dizisine çevirir.
        }
        ...
        {
            byte[] iv = new byte[16]; //IV (Initialization Vector): AES şifrelemede başlangıç değeri.
        ...
        public static string DecryptString(string key, string cipherText) //Bu metodun amacı: Şifrelenmiş string'i tekrar düz yazıya çevirmektir.
        {
            ...
            byte[] buffer = Convert.FromBase64String(cipherText); //Aynı IV’yi (sıfırlarla dolu 16 byte) tekrar kullanmamız gerekir.
